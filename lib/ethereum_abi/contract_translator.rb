...
module EthereumAbi
  class ContractTranslator
    def initialize(contract_interface)
      contract_interface = JSON.parse contract_interface if contract_interface.instance_of?(String)
    ...
    def encode(name, args)
      ...
      calldata = EthereumAbi.encode_abi desc[:encode_types], args
    ...
    def encode_constructor_arguments(args)
      ...
      EthereumAbi.encode_abi constructor_data[:encode_types], args
    end

    def decode(name, data)
      desc = function_data[name]
      EthereumAbi.decode_abi desc[:decode_types], data
    end
    ...
    def listen(log, noprint: false)
      return if log.topics.size.zero? || !event_data.key?(log.topics[0])
      ...
      indexed_types = types.zip(indexed).select { |(_t, i)| i.true? }.map(&:first)
      unindexed_types = types.zip(indexed).select { |(_t, i)| i.false? }.map(&:first)

      deserialized_args = EthereumAbi.decode_abi unindexed_types, log.data
      ...
      names.each_with_index do |n, i|
        if indexed[i].true?
          ...
          o[n] = EthereumAbi.decode_primitive_type ABI::Type.parse(indexed_types[c1]), topic_bytes
          c1 += 1
        else
    ...
    def canonical_name(x)
      case x
      when /\A(uint|int)(\[.*\])?\z/
        "#{Regexp.last_match(1)}256#{Regexp.last_match(2)}"
      when /\A(real|ureal|fixed|ufixed)(\[.*\])?\z/
        "#{Regexp.last_match(1)}128x128#{Regexp.last_match(2)}"
      else
        x
      end
    end
