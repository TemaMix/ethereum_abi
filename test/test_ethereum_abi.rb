...
class TestEthereumAbi < Minitest::Test
  def test_that_it_has_a_version_number
    refute_nil ::EthereumAbi::VERSION
  end

  include EthereumAbi
  include EthereumAbi::Utils

  def on_fixture_test(_name, data)
...
class TestAbi < Minitest::Test
  ...
  include EthereumAbi::Utils
  ABI = EthereumAbi
  Constant = EthereumAbi::Constant

  def test_use_abi_class_methods
    assert_equal encode(["int256"], [1]), EthereumAbi.encode(["int256"], [1])
  end
  ...
  def test_get_int_and_uint
    ...
    # int_to_big_endian fails
    assert_raises(RLP::Error::SerializationError) do
      ABI.send(:get_uint, int_to_big_endian(Constant::UINT_MIN - 1))
    end
  ...
  def test_encode_int
    ...
    assert_raises(EncodingError) { encode_primitive_type(int256, 2**256) }
    assert_raises(EncodingError) { encode_primitive_type(int256, -2**256) }
  end
  ...
  def test_encode_uint
    ...
    assert_raises(EncodingError) { encode_primitive_type(uint8, -1) }
    assert_raises(EncodingError) { encode_primitive_type(uint32, -1) }
    assert_raises(EncodingError) { encode_primitive_type(uint256, -1) }
  ...
  def test_encode_dynamic_string
    ...
    a = "\xC3\xA3".dup.force_encoding("UTF-8")
    a_utf8 = a.b
  ...
  def test_encode_decode_address
    ...
    address_abi = %w[address address address]
